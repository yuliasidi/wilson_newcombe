...
if (check_ymean%>%all()){
   ...
   #MI
  mu_kc <- (1-mean(dtmiss$r[dtmiss$trt=="c"]))/(1 - mean(dtmiss$r[dtmiss$trt=="c" & dtmiss$y==1]))
  mu_kt <- (1-mean(dtmiss$r[dtmiss$trt=="t"]))/(1 - mean(dtmiss$r[dtmiss$trt=="t" & dtmiss$y==1]))
  
   dt_mi <- 
     bind_rows(dtmiss%>%
                 filter(trt=='c')%>%
                 mi(n_mi = num_n_mi, m_mi = num_m_mi, mu_k = mu_kc, sd_k = sd_k, ym = 'y.m')%>%
                 dplyr::mutate(trt='c'),
               dtmiss%>%
                 filter(trt=='t')%>%
                 mi(n_mi = num_n_mi, m_mi = num_m_mi, mu_k = mu_kt, sd_k = sd_k, ym = 'y.m')%>%
                 dplyr::mutate(trt='t'))
...
saveRDS(x1, sprintf("results/p2_mnar_new/p2_mnar_set_new_n%s.rds",
                    set_n))
