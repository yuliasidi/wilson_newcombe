...
setting_ymnar.l <- as.list(setting_ymnar%>%filter(set_n<14)%>%filter(set_n!=7))
...
purrr::pwalk(.l = setting_ymnar.l,
             .f = function(pc, m2, do_rate, n_obs, set_n, mp_y1){
               cat(
                 ...
                 file = file.path('pgms_slurm/p2_mnar_new/params',
                                  sprintf("p2_mnar_set_new_n%s_params.R",
                                          set_n)
                 ),
                 sep='\n')
             })
