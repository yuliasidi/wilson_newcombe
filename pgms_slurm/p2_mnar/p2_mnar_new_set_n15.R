...
###################################################
 ...
 mp_y0_val <- mp_y0(do_tar = do_rate, mp_y1 = mp_y1, p_y1 = pt)
 dtmiss <- dtfull%>%mutate(r=0)

 dtmiss[dtmiss$trt=='c', 'r'] <- stats::rbinom(sum(as.numeric(dtmiss$trt=='c')), 1, prob = do_rate)  
 dtmiss[dtmiss$trt=='t' & dtmiss$y==1, 'r'] <- 
   stats::rbinom(sum(as.numeric(dtmiss$trt=='t' & dtmiss$y==1)), 1, prob = mp_y1)
 dtmiss[dtmiss$trt=='t' & dtmiss$y==0, 'r'] <- 
   stats::rbinom(sum(as.numeric(dtmiss$trt=='t' & dtmiss$y==0)), 1, prob = mp_y0_val)
